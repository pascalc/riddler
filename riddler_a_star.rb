...
=begin
	left_bank = 0
	left_bank += 1 if(!state.D)
	left_bank += 1 if(!state.C)
	left_bank += 1 if(!state.B)
	left_bank += 1 if(!state.A)	
	return left_bank
=end
	# Average travel time for people on the left bank -- 526 node expansions
=begin
	return 0 if left_bank == 0
	travel_time = 0
	travel_time += D_cost if(!state.D)
	travel_time += C_cost if(!state.C)
	travel_time += B_cost if(!state.B)
	travel_time += A_cost if(!state.A)	
	return (travel_time/left_bank)	
=end
end
...
until q.empty? do
	...
	if(node.A == lantern_side)
		...
		q.push(fringe_node.dup,fringe_node.running_cost + heuristic(fringe_node))
